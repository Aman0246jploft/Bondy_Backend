...
const getAdminStats = async () => {
    try {
            ...
            User.aggregate([
                {
                    $group: {
                        _id: null,
                        totalUsers: { $sum: 1 },
                        totalOrganizers: { $sum: { $cond: [{ $eq: ["$roleId", roleId.ORGANISER] }, 1, 0] } },
                        totalCustomers: { $sum: { $cond: [{ $eq: ["$roleId", roleId.CUSTOMER] }, 1, 0] } },
                    }
                }
            ]),
            Event.aggregate([
                {
                    $group: {
                        _id: null,
                        totalEvents: { $sum: 1 },
                        upcomingEvents: { $sum: { $cond: [{ $eq: ["$status", "Upcoming"] }, 1, 0] } },
        ...
        const stats = {
            users: userStats[0] || { totalUsers: 0, totalOrganizers: 0, totalCustomers: 0 },
            events: eventStats[0] || { totalEvents: 0, upcomingEvents: 0, liveEvents: 0 },
            finance: transactionStats[0] || { totalRevenue: 0, totalCommission: 0, totalTicketsSold: 0 }
        };
...
const getOrganizerStats = async (organizerId) => {
    try {
        ...
        const stats = {
            events: eventStats[0] || { totalEvents: 0, upcomingEvents: 0, liveEvents: 0, pastEvents: 0 },
            performance: transactionStats[0] || { totalEarnings: 0, totalTicketsSold: 0, totalBookings: 0 }
        };
