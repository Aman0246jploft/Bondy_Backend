...
const createReport = async (req, res) => {
    try {
        ...
        if (fromUser === toUser) {
            return apiErrorRes(HTTP_STATUS.BAD_REQUEST, res, "You cannot report yourself.");
        }

        const existingReport = await Report.findOne({ fromUser, toUser });
        if (existingReport) {
            return apiErrorRes(HTTP_STATUS.BAD_REQUEST, res, "You have already reported this user.");
        }
        ...
        return apiSuccessRes(HTTP_STATUS.OK, res, "User reported successfully.", newReport);
    } catch (error) {
...
const { reportUserSchema } = require("../services/validations/adminValidations");

router.post("/create", perApiLimiter(), validateRequest(reportUserSchema), createReport);
