...
const deleteContact = async (req, res) => {
    try {
        const { id } = req.body;
        if (!id) {
            return apiErrorRes(HTTP_STATUS.BAD_REQUEST, res, "Contact ID is required");
        }
...
const updateContact = async (req, res) => {
    try {
        ...
        if (!id || !status) {
            return apiErrorRes(HTTP_STATUS.BAD_REQUEST, res, "Contact ID and Status are required");
        }
        ...
        return apiSuccessRes(HTTP_STATUS.OK, res, "Contact status updated successfully", contact);
    } catch (error) {
        return apiErrorRes(HTTP_STATUS.SERVER_ERROR, res, error.message);
    }
};
