...
/**
 * Populate qrs[] on each ticket in transaction.tickets[]
 * so every INDIVIDUAL ticket has its own subBookingId and qrCodeData,
 * while keeping the tickets array grouped by ticket type.
 *
 * Example:
 *   tickets = [
 *     { 
  ...
  try {
    ...
    if (!batchId) {
      ...
      const attendees = paginated.map(({ txn, allTxns }) => {
        ...
        return {
          ...
          // organizerEarning: txn.organizerEarning || 0,
          enrolledBatches,
          user: {
            _id: user?._id,
            firstName: user?.firstName,
      ...
      return apiSuccessRes(
        ...
        constantsMessage.ATTENDEE_LIST_FETCHED || "Attendees list fetched successfully",
        {
          course: {
            ...
            courseType: course.enrollmentType,
          },
    ...
    const attendees = transactions.map((transaction) => {
      ...
      return {
        ...
        qrCodeData: transaction.qrCodeData,
        totalAmount: transaction.totalAmount || 0,
        basePrice: transaction.basePrice || 0,
        taxAmount: transaction.taxAmount || 0,
        discountAmount: transaction.discountAmount || 0,
        // commissionAmount: transaction.commissionAmount || 0,
        // organizerEarning: transaction.organizerEarning || 0,
        batchDetails,
