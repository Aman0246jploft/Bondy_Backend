...
const uploadVerificationDoc = async (req, res) => {
    try {
        ...
        const uploadPromises = req.files.map((file) => storeImage(file, `verification/${userId}`));
...
const submitVerification = async (req, res) => {
    try {
        const userId = req.user.userId;
        const { idVerification, contactVerification, payoutVerification, businessVerification } = req.body;
        ...
        if (idVerification) verification.idVerification = { ...verification.idVerification, ...idVerification, status: "pending", date: new Date() };
        if (contactVerification) verification.contactVerification = { ...verification.contactVerification, ...contactVerification, status: "pending", date: new Date() };
        if (payoutVerification) verification.payoutVerification = { ...verification.payoutVerification, ...payoutVerification, status: "pending", date: new Date() };
        if (businessVerification) verification.businessVerification = { ...verification.businessVerification, ...businessVerification, status: "pending", date: new Date() };
        ...
        return apiSuccessRes(HTTP_STATUS.OK, res, "Verification submitted successfully.", verification);
    } catch (error) {
        return apiErrorRes(HTTP_STATUS.INTERNAL_SERVER_ERROR, res, error.message, error.message);
    }
};
...
const getVerificationStatus = async (req, res) => {
    try {
        ...
        if (!verification) {
            return apiSuccessRes(HTTP_STATUS.OK, res, "No verification record found.", null);
        }

        return apiSuccessRes(HTTP_STATUS.OK, res, "Verification status fetched.", verification);
    } catch (error) {
        return apiErrorRes(HTTP_STATUS.INTERNAL_SERVER_ERROR, res, error.message, error.message);
    }
};
...
const updateVerificationStatus = async (req, res) => {
    try {
        ...
        if (req.user.roleId !== roleId.SUPER_ADMIN && req.user.roleId !== roleId.ADMIN) {
        ...
        if (!verification) {
            return apiErrorRes(HTTP_STATUS.NOT_FOUND, res, "Verification record not found.");
        }
        ...
        else if (type === 'business') updateField(verification.businessVerification);
        else {
            return apiErrorRes(HTTP_STATUS.BAD_REQUEST, res, "Invalid verification type.");
        }
        ...
        return apiSuccessRes(HTTP_STATUS.OK, res, "Verification status updated.", verification);
    } catch (error) {
        return apiErrorRes(HTTP_STATUS.INTERNAL_SERVER_ERROR, res, error.message, error.message);
    }
};
...
const { verifyStatusSchema } = require("../services/validations/adminValidations");
...
router.post("/update-status", perApiLimiter(), validateRequest(verifyStatusSchema), updateVerificationStatus);
